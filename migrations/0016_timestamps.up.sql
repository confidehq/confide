...
--
-- The whole body runs only on first application. The ALTER COLUMN TYPE steps are
-- naturally idempotent, but the UPDATE backfills are not: re-running them would
-- reset every updated_at to its created_at. accounts.updated_at is the sentinel.
DO $$
BEGIN
    IF EXISTS (
