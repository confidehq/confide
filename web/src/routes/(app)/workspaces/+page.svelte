...
<div class="font-mono w-full max-w-3xl md:max-w-4xl lg:max-w-5xl xl:max-w-6xl px-4 pt-10 pb-12 sm:px-8 sm:pt-10">
	...
	<div class="flex items-start justify-between mb-8 gap-4">
		<h1 class="text-2xl m-0 text-text-bright font-semibold">All Workspaces</h1>
		{#if !showCreate}
			<button
	...
	{#if showCreate}
		<div class="mb-8 p-5 border border-border-deep rounded-lg flex flex-col gap-4">
			<div class="flex items-center justify-between gap-2">
				<span class="text-base text-text-body font-semibold">New workspace</span>
				<button
					onclick={cancelCreate}
					class="bg-transparent border-none cursor-pointer text-muted-dim hover:text-text-body transition-colors duration-100 p-1 rounded"
					aria-label="Cancel"
			...
			<div class="flex gap-2">
				...
				<button
					onclick={cancelCreate}
					class="px-4 py-2 bg-transparent text-muted-dim border border-border-deep rounded cursor-pointer font-mono text-base hover:text-text-body hover:border-border-subtle transition-colors duration-100"
				>Cancel</button>
			</div>
		</div>
	{/if}
	...
	{:else if workspaces.length === 0}
		<div class="py-12 border border-dashed border-border rounded-lg text-center">
			<p class="m-0 text-muted-mid text-base">No workspaces found</p>
		</div>
	{:else}
			...
			{#each workspaces as ws (ws.id)}
				...
				<div>
					...
					<div class="flex items-center gap-3 mb-4">
						...
						<h2 class="m-0 text-xl font-semibold text-text-bright truncate min-w-0">{ws.name}</h2>
						...
						<div class="ml-auto flex items-center gap-2 shrink-0">
							...
							<div class="relative">
								...
								<button
									...
									class="flex items-center justify-center w-8 h-8 bg-transparent border rounded cursor-pointer text-muted-dim transition-colors duration-100
										{openMenuId === ws.id
											? 'text-text-body border-border-subtle bg-surface-3'
											: 'border-border-deep hover:text-text-body hover:border-border-subtle'}"
					...
					<!-- Forms list -->
					{#if formsLoading}
						<div class="py-5 text-muted-mid text-base">Loading forms…</div>
					{:else if forms.length === 0}
						<div class="py-8 border border-dashed border-border rounded-lg text-center">
							<p class="m-0 text-muted-mid text-base">No forms in this workspace</p>
						</div>
					{:else}
						<div class="border border-border-deep rounded-lg overflow-hidden">
							{#each forms as form, i (form.formId)}
								...
								>
									...
									<span class="flex-1 min-w-0 text-base text-text-body truncate">
