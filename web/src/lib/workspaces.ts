...
async function getOrCreateIdentityKey(
	...
	if (res.status !== 404) {
		throw new WorkspaceError(
			"identity_key_fetch",
			`Failed to fetch identity key (${res.status})`,
		);
	}
	...
	if (!put.ok) {
		throw new WorkspaceError(
			"identity_key_save",
			`Failed to save identity key (${put.status})`,
		);
	}
...
export async function getWorkspaceSettings(
	...
	if (!res.ok) {
		...
		const message =
			...
			`Failed to get settings (${res.status})`;
...
export async function renameWorkspace(id: string, name: string): Promise<void> {
	...
	if (!res.ok && res.status !== 204) {
		...
		const message =
			(body as { message?: string }).message ??
			`Failed to rename workspace (${res.status})`;
		throw new WorkspaceError(code, message);
	}
}
...
export async function deleteWorkspace(id: string): Promise<void> {
	...
	if (!res.ok && res.status !== 204) {
		...
		const message =
			(body as { message?: string }).message ??
			`Failed to delete workspace (${res.status})`;
		throw new WorkspaceError(code, message);
	}
}
...
export async function getCustomDomain(
	...
	if (!res.ok) {
		...
		const message =
			(body as { message?: string }).message ??
			`Failed to get custom domain (${res.status})`;
		throw new WorkspaceError(code, message);
	}
...
export async function setCustomDomain(
	...
	if (!res.ok) {
		...
		const message =
			(body as { message?: string }).message ??
			`Failed to set custom domain (${res.status})`;
		throw new WorkspaceError(code, message);
	}
	return res.json() as Promise<CustomDomainInfo>;
}
...
export async function verifyCustomDomain(
	...
	if (!res.ok) {
		...
		const message =
			(body as { message?: string }).message ??
			`Failed to verify custom domain (${res.status})`;
		throw new WorkspaceError(code, message);
	}
	return res.json() as Promise<CustomDomainInfo>;
}
...
export async function clearCustomDomain(workspaceId: string): Promise<void> {
	...
	if (!res.ok && res.status !== 204) {
		...
		const message =
			(body as { message?: string }).message ??
			`Failed to remove custom domain (${res.status})`;
		throw new WorkspaceError(code, message);
	}
}
...
export async function getBillingInfo(
	...
	if (!res.ok) {
		...
		const message =
			(body as { message?: string }).message ??
			`Failed to load billing info (${res.status})`;
		throw new WorkspaceError(code, message);
	}
...
export async function subscribe(
	...
	const res = await apiFetch(
		`/api/workspaces/${workspaceId}/billing/subscribe`,
		{
			method: "POST",
			headers: { "Content-Type": "application/json" },
	...
	if (!res.ok) {
		...
		const message =
			(body as { message?: string }).message ??
			`Failed to start checkout (${res.status})`;
		throw new WorkspaceError(code, message);
	}
...
export async function listWorkspaces(): Promise<Workspace[]> {
	...
	if (!res.ok)
		throw new WorkspaceError(
			"list_failed",
			`Failed to load workspaces (${res.status})`,
		);
	const body = await res.json();
...
export async function listMembers(
	...
	if (!res.ok) {
		...
		const message =
			(body as { message?: string }).message ??
			`Failed to load members (${res.status})`;
		throw new WorkspaceError(code, message);
	}
	const body = await res.json();
	return body.members ?? [];
...
export async function createInvitation(
	...
	if (!res.ok) {
		...
		const message =
			(body as { message?: string }).message ??
			`Failed to send invitation (${res.status})`;
		throw new WorkspaceError(code, message);
	}
...
export async function listInvitations(
	...
	if (!res.ok) {
		...
		const message =
			(body as { message?: string }).message ??
			`Failed to load invitations (${res.status})`;
		throw new WorkspaceError(code, message);
	}
	const body = await res.json();
...
export async function revokeInvitation(
	...
	if (!res.ok && res.status !== 204) {
		...
		const message =
			(body as { message?: string }).message ??
			`Failed to revoke invitation (${res.status})`;
		throw new WorkspaceError(code, message);
	}
}
...
export async function resolveInvitation(token: string): Promise<InvitePreview> {
	...
	if (res.status === 404)
		throw new WorkspaceError("not_found", "Invitation not found.");
	if (res.status === 410)
		throw new WorkspaceError(
			"expired",
	...
	if (!res.ok)
		throw new WorkspaceError(
			"resolve_failed",
			`Failed to load invitation (${res.status})`,
		);
...
export async function listPendingGrants(
	...
	const res = await apiFetch(
		`/api/workspaces/${workspaceId}/pending-key-grants`,
	);
	if (!res.ok) {
		const body = await res.json().catch(() => ({}));
		const code = (body as { code?: string }).code ?? "list_failed";
		const message =
			(body as { message?: string }).message ??
			`Failed to load pending grants (${res.status})`;
		throw new WorkspaceError(code, message);
	}
	const body = await res.json();
	return body.pending ?? [];
...
export async function listMemberIdentityKeys(
	workspaceId: string,
): Promise<MemberIdentityKey[]> {
	const res = await apiFetch(
		`/api/workspaces/${workspaceId}/members/identity-keys`,
	);
	if (!res.ok) {
		const body = await res.json().catch(() => ({}));
		const code = (body as { code?: string }).code ?? "list_failed";
		const message =
			(body as { message?: string }).message ??
			`Failed to load identity keys (${res.status})`;
		throw new WorkspaceError(code, message);
	}
	const body = await res.json();
	return body.members ?? [];
...
export async function grantMemberKey(
	...
	const message =
		(body as { message?: string }).message ??
		`Failed to grant key (${res.status})`;
	throw new WorkspaceError(code, message);
}
...
export async function grantKey(
	...
	if (!ikRes.ok)
		throw new WorkspaceError(
			"identity_key",
			"Failed to load your identity key",
		);
	const ikBody = (await ikRes.json()) as { wrappedIdentityPrivateKey: string };

	// 2. Decrypt identity private key
	const wrappedPrivBlob = base64ToBytes(ikBody.wrappedIdentityPrivateKey)
		.buffer as ArrayBuffer;
	const identityPrivKey = await unwrapIdentityPrivateKey(
		wrappedPrivBlob,
		masterKey,
	);
	...
	if (!wkRes.ok)
		throw new WorkspaceError("workspace_key", "Failed to load workspace key");
	const wkBody = (await wkRes.json()) as {
		wrappedWorkspaceKey: string;
		ephemeralPublicKey: string;
	};

	const wrappedKey = base64ToBytes(wkBody.wrappedWorkspaceKey)
		.buffer as ArrayBuffer;
	const ephPub = base64ToBytes(wkBody.ephemeralPublicKey).buffer as ArrayBuffer;
	const recipientPub = base64ToBytes(targetIdentityPubKeyB64)
		.buffer as ArrayBuffer;
...
export async function loadWorkspaceKey(
	...
	const ikRes = await apiFetch("/api/account/identity-key");
	if (!ikRes.ok)
		throw new WorkspaceError("identity_key", "Failed to load identity key");
	const ikBody = (await ikRes.json()) as { wrappedIdentityPrivateKey: string };
	const wrappedPrivBlob = base64ToBytes(ikBody.wrappedIdentityPrivateKey)
		.buffer as ArrayBuffer;
	const identityPrivKey = await unwrapIdentityPrivateKey(
	...
	if (!wkRes.ok)
		throw new WorkspaceError(
			"workspace_key",
			"No workspace key — not yet granted access",
		);
	const wkBody = (await wkRes.json()) as {
		wrappedWorkspaceKey: string;
		ephemeralPublicKey: string;
...
export async function createWorkspace(
	...
	if (!createRes.ok) {
		...
		const message =
			(createBody as { message?: string }).message ??
			`Failed to create workspace (${createRes.status})`;
		throw new WorkspaceError(code, message);
	}
	...
	const { wrappedWorkspaceKey, ephemeralPublicKey } =
		await generateAndWrapWorkspaceKey(
			identityPublicKey,
			enc.encode(workspace.id),
		);
...
export async function createProWorkspace(
	...
	if (!keyRes.ok && keyRes.status !== 204) {
		throw new WorkspaceError(
			...
			`Workspace created but key upload failed (${keyRes.status})`,
		);
	}
 ...
 */
export async function setupPersonalWorkspaceKey(
	masterKey: CryptoKey,
	accountId: string,
): Promise<void> {
	...
	if (!res.ok && res.status !== 204) {
		throw new WorkspaceError(
			"setup_key_failed",
			`Failed to set up workspace key (${res.status})`,
		);
	}
}
