...
const isNonDefaultLocale = $derived(
	store.activeLocale !== store.schema.defaultLocale,
);
...
function setOptionLabel(index: number, value: string) {
	if (!fieldId) return;
	const current =
		currentFieldTranslation?.options ?? Array(optionsCount).fill("");
