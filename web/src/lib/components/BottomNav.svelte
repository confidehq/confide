...
{#if moreOpen}
	<div
		...
		class="sm:hidden fixed bottom-16 left-0 right-0 z-50 bg-base border-t border-border border-b border-border rounded-t-2xl font-mono
	...
	>
		...
		<div class="px-4 py-2 border-b border-canvas">
			{#if showWorkspacePicker}
				...
				<div class="flex items-center gap-2 mb-3">
					...
					<span class="text-xs uppercase tracking-widest text-subtle font-medium">Workspaces</span>
				</div>

				{#if showCreateWorkspace}
					<div class="mb-3">
						<p class="m-0 mb-2 text-xs uppercase tracking-widest text-subtle font-medium">New workspace</p>
						{#if access.atWorkspaceLimit}
						...
						<div class="flex gap-2">
							<button
								...
								class="flex-1 py-2 text-sm text-white border-none rounded cursor-pointer font-mono transition-colors duration-100
									{creating || !newWorkspaceName.trim() ? 'bg-muted cursor-not-allowed' : 'bg-primary hover:bg-primary-hover'}"
							...
							<button
								...
								class="px-3 py-2 text-sm text-subtle bg-transparent border border-border rounded cursor-pointer font-mono hover:text-text transition-colors duration-100"
							>Cancel</button>
						</div>
					</div>
				{:else}
						...
						{#each workspacesStore.workspaces as ws (ws.id)}
							...
							<button
								...
								class="w-full flex items-center gap-2.5 px-2 py-2 text-left bg-transparent border-none cursor-pointer font-mono rounded-md transition-colors duration-100
									{active ? 'bg-canvas' : 'hover:bg-surface'}"
							>
								<span class="shrink-0 w-7 h-7 rounded flex items-center justify-center bg-base border border-border text-text text-sm font-semibold">
									{initials(ws.name)}
								</span>
			...
			{:else}
				...
				>
					<span class="shrink-0 w-8 h-8 rounded-md flex items-center justify-center bg-canvas border border-border text-text text-sm font-semibold select-none">
		...
		{#if !showWorkspacePicker}
			<div class="py-1.5">
				...
				<div class="border-t border-canvas mx-4 my-1"></div>
...
<nav
	style="padding-bottom: env(safe-area-inset-bottom, 0px);"
	class="sm:hidden fixed bottom-0 left-0 right-0 z-40 bg-base border-t border-canvas font-mono"
>
