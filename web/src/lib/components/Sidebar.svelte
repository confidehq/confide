<script lang="ts">
	...
	import { LayoutGrid, FileText, Settings, ChevronLeft, ChevronRight, MessageSquare, LogOut } from '@lucide/svelte';
...
>
	...
	>
		...
		{:else}
			<span class="text-text-dim text-lg font-semibold tracking-tight">confide</span>
			<button
				onclick={() => sidebar.toggle()}
			...
			{#each links as link}
				...
				<a
					...
					class="flex items-center gap-2.5 h-10 no-underline whitespace-nowrap overflow-hidden
						text-sm box-border w-full transition-[color,background] duration-100
						{active
